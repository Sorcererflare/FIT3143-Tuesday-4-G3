...
#define KILL_TAG          3
#define REBORN_TAG        4   // NEW (reborn): master -> invader "you respawn now"
...
int main(int argc, char *argv[]) {
    ...
    if (rank == MASTER) {
        ...
        while (!game_over) {
            ...
            for (int i = 1; i <= n * m; i++) {
                int fired;
                MPI_Recv(&fired, 1, MPI_INT, i, INVADER_SHOT_TAG, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                ...
                for (int row = n - 1; row >= 0; row--) {
                    ...
                    if (time_traveled == time_needed) {
                        ...
                        if (invaders[target_index].alive) {
                            ...
                            if (is_bottom_most) {
                                printf(">>> HIT! Player shot reached invader at (%d,%d)\n",
                                       row, player_cannonballs[i].col);

                                // Tell invader it was hit so it can apply ricochet/blocked/death
                                int inv_rank = 1 + target_index;
                                int shot = 1;
                                MPI_Send(&shot, 1, MPI_INT, inv_rank, PLAYER_SHOT_TAG, MPI_COMM_WORLD);

                                // We let the invader decide what happens (ricochet/blocked/die)
                                // and we'll observe the status on the next tick.
            ...
            // 6.5 REBORN SWEEP (every 5 ticks)  ------------------  NEW (reborn)
            if (tick % 5 == 0) {
                for (int row = 0; row < n; ++row) {
                    for (int col = 0; col < m; ++col) {
                        int idx = row * m + col;
                        if (invaders[idx].alive) continue;  // only consider dead ones

                        int left_alive  = (col > 0)     ? invaders[row*m + (col-1)].alive : 0;
                        int right_alive = (col < m-1)   ? invaders[row*m + (col+1)].alive : 0;

                        if (left_alive && right_alive) {
                            if ((rand() % 100) < 20) {    // 20% chance
                                // Update master's board immediately so print reflects it this tick
                                invaders[idx].alive = 1;

                                // Notify that specific invader process to flip alive=1
                                int inv_rank = 1 + idx;   // player=0, invaders=1..n*m
                                int one = 1;
                                MPI_Send(&one, 1, MPI_INT, inv_rank, REBORN_TAG, MPI_COMM_WORLD);

                                printf("REBORN: invader at (%d,%d)\n", row, col);
                            }
                        }
                    }
                }
            ...
            print_game(n, m, invaders, player_col,
                       player_cannonballs, num_player_cannonballs,
                       enemy_cannonballs, num_enemy_cannonballs);
        ...
        while (1) {
            ...
            MPI_Send(&fired, 1, MPI_INT, MASTER, INVADER_SHOT_TAG, MPI_COMM_WORLD);

            int flag;
            MPI_Status status;

            // 3. Invader has been shot by the player -> apply ricochet/blocked/death
            ...
            if (flag) {
                ...
                int random = rand() % 100;
                if (random < 20) {
                    // redirect left (if valid neighbour)
                    if (left != MPI_PROC_NULL) {
                        int kill = 1;
                        MPI_Send(&kill, 1, MPI_INT, left, KILL_TAG, MPI_COMM_WORLD);
                    }
                    // this invader survives
                } else if (random < 35) {
                    // redirect right (if valid neighbour)
                    ...
                    // blocked by shield -> survive
                    alive = alive; // no-op, just explicit
                } else {
                    // intended target is killed (45%)
                    alive = 0;
                }
            }

            // 3.x Reborn from master  ---------------------------  NEW (reborn)
            MPI_Iprobe(MASTER, REBORN_TAG, MPI_COMM_WORLD, &flag, &status);
            if (flag) {
                int one;
                MPI_Recv(&one, 1, MPI_INT, MASTER, REBORN_TAG, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                alive = 1;
            }
            // -----------------------------------------------------

            // 4. Invader has been killed by neighbour ricochet
            if (left != MPI_PROC_NULL) {
